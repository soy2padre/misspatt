...
#' @describeIn RNGseq Creates a given number of seeds for L'Ecuyer's RNG, that can be used to seed 
...
RNGkind_default <- function(){
  RNGkind(kind = "default", normal.kind = "default", sample.kind = "default", strict = FALSE)
  
}

#' @describeIn RNGseq generates the -- next -- random seed used as the first seed in 
#' the sequence generated by \code{\link{RNGseq}}. 
#' 
#' @param normal.kind Type of Normal random generator passed to [base::RNGkind]. See \code{\link{RNG}}.
#' @param sample.kind Type of Discrete Uniform random generator passed to [base::RNGkind]. See \code{\link{RNG}}.
...
#'  
RNGseq_seed <- function(seed=NULL, normal.kind = NULL, sample.kind = NULL, verbose=FALSE, version=2){
  ...
  if( is.numeric(seed) ){
    if( length(seed) == 1L ){
      ...
      if( version<2 || rkind_not_CMRG ){ # behaviour prior 1.4
        set.seed(seed)
        RNGkind(kind="L'Ecuyer-CMRG", normal.kind=normal.kind, sample.kind = sample.kind)
      ...
      RNGkind("L'Ecuyer-CMRG", normal.kind=normal.kind, sample.kind = sample.kind)
      s <- RNGseed()
    ...
    if( rkind_not_CMRG ){ # seed with random seed
      ...
      RNGkind(kind="L'Ecuyer", normal.kind=normal.kind, sample.kind = sample.kind)
      if( verbose ) message("OK")
      ...
      if( version < 2 ){
        ...
        if( !is.null(normal.kind) || !is.null(sample.kind) ) RNGkind(normal.kind=normal.kind, sample.kind = sample.kind)
